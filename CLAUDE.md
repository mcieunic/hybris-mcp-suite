...
- `packages/shared` — common helpers: `loadEnvFile(pkg)` (reads `.env` per package), arg validators (`validateString` / `validateNumber` / `validateBoolean` / `validateStringArray`).
- `packages/runtime` — **HAC-based** tools. Owns `HybrisClient` (browser-style session against HAC), log readers (local + Azure blob), storefront client, CMS export. Server name: `hybris-mcp-runtime`. Tools: `execute_groovy`, `flexible_search`, `describe_type`, `import_impex`, `export_impex`, `get_cronjobs`, `trigger_cronjob`, log/storefront/CMS tools.
- `packages/solr` — **direct Solr HTTP** tools. Owns `SolrClient`. Server name: `hybris-mcp-solr`. Tools: `solr_list_cores`, `solr_core_info`, `solr_query`, `solr_schema_fields`, `solr_reload_core`, `solr_swap_core`, `solr_backup_core`, `solr_restore_core`, `solr_backup_status`, `solr_restore_status`, `solr_schema_add_field`.
...
- **HAC vs direct**: runtime hits HAC (`/console/scripting/execute`, FlexibleSearch, ImpEx). solr hits Solr over HTTP. They do not share clients. If a feature needs HAC, it lives in runtime, even if it's logically "about Solr" — e.g. listing CCV2 cores via `solrServerService.getSolrServer` belongs in runtime, not solr.
...
- runtime: HAC creds + `HYBRIS_LOG_PATH` (local logs) + Azure blob log vars (`AZURE_BLOB_LOG_SAS_URL` or `AZURE_BLOB_LOG_ACCOUNT_NAME` + key + container; cache dir from `AZURE_BLOB_LOG_CACHE_DIR` or falls back to `HYBRIS_LOG_PATH`). Log tools throw a clear "disabled — set X" error when env is missing.
- solr: `SOLR_URL` (default `http://localhost:8983/solr/`), `SOLR_USERNAME`, `SOLR_PASSWORD`.
